#!/usr/bin/env bash
# Capture the Ch 15 (persistent sessions / tmux) fact that the
...
# material (ch15-coreloop.txt, ch15-progress.txt,
# ch15-structure.txt, ch15-sendkeys.txt, ch15-screen.txt): the
...
#
#   1. ch15-versions-mac.txt - the Mac's tmux (and screen)
#                              version, for the DIVERGENCE / stamp.
#   2. ch15-server-mac.txt   - the survive-a-disconnect demo. A
...
#                              (Ch 11). The session is killed and
...
#
#   bash transcripts/capture-ch15-mac.sh <ssh-alias>
#   # e.g. bash transcripts/capture-ch15-mac.sh lab-server
#
# then paste back (or say "done") and the chapter is reconciled
# against the two ch15-*-mac.txt files byte-for-byte.
#
# PRIVACY: this TRACKED script hardcodes NO personal data. It
# derives the server hostname, account, home, and any literal IP
# at runtime and MASKS them at capture time (the Ch 14 mask()),
...
# hang, the Ch 10 lesson). It writes only under transcripts/ and a
# throwaway ~/ch15-job.log on the server, removed at the end.
...
SERVER="${1:?usage: bash transcripts/capture-ch15-mac.sh <ssh-alias>}"
...
# =============================================================
{
  hdr "macOS tmux + screen" \
    ...
    "version stamp; the Linux side is in ch15-screen.txt. No" \
    "personal data in these version lines."
  echo "\$ tmux -V"
  tmux -V 2>&1 | mask
  echo "\$ screen --version"
  screen --version 2>&1 | mask
} > "$here/ch15-versions-mac.txt"
...

{
  ...
  echo "\$ ssh lab-server 'tmux new-session -d -s job \"bash ch15-job.sh\"'"
  ssh "${SSHO[@]}" "$SERVER" \
    'tmux new-session -d -s job "bash ch15-job.sh"' 2>&1 | mask
  sleep 5
  echo ""
  ...
  ssh "${SSHO[@]}" "$SERVER" 'tmux capture-pane -t job -p' 2>&1 \
    | strip | mask
  echo ""
  echo "# The Ch 11 log survives independently of the session:"
  echo "\$ ssh lab-server 'tail -3 ~/ch15-job.log'"
  ssh "${SSHO[@]}" "$SERVER" 'tail -3 ~/ch15-job.log' 2>&1 | mask
  echo ""
...
} > "$here/ch15-server-mac.txt"

ssh "${SSHO[@]}" "$SERVER" 'rm -f ~/ch15-job.sh ~/ch15-job.log' 2>/dev/null
