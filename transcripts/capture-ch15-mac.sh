#!/usr/bin/env bash
# Capture the Ch 15 (Modern CLI kit & TUIs) facts on the user's
# Mac. The whole kit is blocked in the book's Linux sandbox
# (GitHub-release / cargo / brew binaries; only ripgrep and du
# are preinstalled there, and both are already taught in Ch 8 and
# Ch 13), so like Ch 16 this is a Mac-heavy capture chapter.
#
...
#      ncdu, fd) into ch15-versions-mac.txt. Any tool that is not
...
#      PDF drops (the Ch 5 box-glyph and Ch 7 duckbox lessons).
...
#   bash transcripts/capture-ch15-mac.sh
# then review the ch15-*-mac.txt files it writes and paste them
...
vout="$here/ch15-versions-mac.txt"
eout="$here/ch15-eza-mac.txt"
bout="$here/ch15-bat-mac.txt"
nout="$here/ch15-nav-mac.txt"
dout="$here/ch15-diff-mac.txt"
...
}
{
  hdr "modern CLI kit + TUIs (version anchor)" \
    ...
    "only evidence (the Ch 16 quarantine pattern)."
  for t in eza bat delta zoxide fzf lazygit btop ncdu fd; do
    ver "$t"
  done
} >"$vout"
...
# same one-line change Ch 16 used). Removed on exit.
scratch="$(mktemp -d "${TMPDIR:-/tmp}/ch15mac.XXXXXX")"
