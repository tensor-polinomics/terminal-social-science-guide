#!/usr/bin/env bash
# Capture the Ch 11 (Scripts that fail loudly) macOS/BSD facts on
# the user's Mac.
#
# Ch 11's portable/GNU output (the shebang, set -euo pipefail,
# exit codes, the trap, the set -e footguns, bash -x, tee,
# /usr/bin/time -v, LC_ALL/TZ pinning, and the verify.sh anchor)
# is captured in the book's Linux sandbox (ch11-*.txt). This
...
#   1. ch11-lint-mac.txt   - ShellCheck + shfmt. Both are
#      GitHub-release binaries blocked in the sandbox; they behave
#      the same cross-platform, so the Mac capture is the evidence
#      (like Ch 8 DuckDB / Ch 9 fd). shellcheck flags the SC2086
#      unquoted expansion Ch 7 warned about; shfmt shows a format
#      diff.
#   2. ch11-bash32-mac.txt - the macOS bash-3.2 floor. macOS ships
...
#      mapfile).
#   3. ch11-bsd-mac.txt    - BSD /usr/bin/time -l (macOS has no
#      GNU -v) and BSD `date -r EPOCH` (GNU uses `date -d @EPOCH`).
#
# Run on the Mac, from the terminal repo root
# (source-private/terminal), the way ch05/ch08/ch09/ch10 were run:
#   bash transcripts/capture-ch11-mac.sh
# then review the three ch11-*-mac.txt files it writes and paste
...
lout="$here/ch11-lint-mac.txt"
bout="$here/ch11-bash32-mac.txt"
sout="$here/ch11-bsd-mac.txt"
...

{
  hdr "ShellCheck ${sc_ver:-?} / shfmt ${sf_ver:-?}" \
    ...
    "capture is the evidence (Ch 8 DuckDB / Ch 9 fd pattern). No" \
