#!/usr/bin/env bash
# Capture the Ch 13 (SSH & remote compute) facts that the Linux
...
# material (ch13-versions.txt, ch13-rsync-move.txt, ch13-keygen.txt,
# ch13-bundle.txt, ch13-space.txt): rsync's trailing-slash and
...
#
#   1. ch13-versions-mac.txt   - the Mac's ssh + rsync versions
#                                (macOS ships openrsync, the
#                                DIVERGENCE against Linux GNU rsync).
#   2. ch13-divergence-mac.txt - probe whether GNU-only rsync flags
...
#   3. ch13-config-mac.txt     - the effective ~/.ssh/config for the
#                                server alias (via ssh -G) and the
#                                local ~/.ssh permissions.
#   4. ch13-server-mac.txt     - ssh one-shots on the server:
#                                uname, df, and ~/.ssh permissions.
#   5. ch13-transfer-mac.txt   - a real rsync push of the project
#                                (excluding .venv/ and renv/library/,
#                                the Ch 12 "commit the lock, not the
#                                environment" line) into a throwaway
#                                ~/ch13-demo, a checksum of the moved
#                                data on both machines (shasum -a 256
#                                vs sha256sum), and a pull back.
#   6. ch13-restore-mac.txt    - rebuild the environment on arrival:
#                                uv sync (fast) and renv::restore()
#                                (compiles from the lock; the long
#                                pole, ~15 min).
#   7. ch13-tunnel-mac.txt     - an ssh -L port forward to a
...
# ~/.ssh/config):
#
#   bash transcripts/capture-ch13-mac.sh <ssh-alias>
#   # e.g. bash transcripts/capture-ch13-mac.sh lab-server
#
# then paste back (or say "done") and the chapter is reconciled
# against the seven ch13-*-mac.txt files byte-for-byte.
#
...
# (the Ch 12 mask() plus a remote-identifier set), so the server's
# identity never lands in the repo. All ssh calls are
# non-interactive one-shots (BatchMode; no interactive remote
# shell, so nothing hangs over ssh, the Ch 9 lesson). It writes
# only under transcripts/ and a throwaway ~/ch13-demo on the
...
SERVER="${1:?usage: bash transcripts/capture-ch13-mac.sh <ssh-alias>}"
...
# =============================================================
{
  hdr "macOS OpenSSH + openrsync" \
    ...
    "ch13-versions.txt. No personal data in these version lines."
  echo "\$ ssh -V"
  ssh -V 2>&1 | mask
  echo "\$ rsync --version | head -2"
  rsync --version 2>&1 | head -2 | mask
} > "$here/ch13-versions-mac.txt"
...
pullback="$(mktemp -d)"
{
  hdr "rsync push/pull + cross-machine checksum" \
    ...
    "(Ch 12). Then the moved CSVs are checksummed on both ends" \
    "(Mac shasum -a 256 vs server sha256sum) to prove the bytes" \
    "survived (Ch 10's manifest idea, across machines). All paths" \
...
# =============================================================
{
  hdr "uv sync + renv::restore() on the server" \
    "The Ch 12 payoff, live on the server: uv sync rebuilds the" \
    ...
    "proceeds with a version notice, itself the Ch 12 point that" \
