#!/usr/bin/env bash
# Capture the Ch 13 (Environments from the shell) macOS facts on
# the user's Mac.
#
# Ch 13's uv output (uv sync / uv run / uv add / uv pip list, the
# .venv tree, and every lockfile read) is captured in the book's
# Linux sandbox (ch13-*.txt). This script captures the things the
# sandbox cannot: R, renv, and sessionInfo() are Mac-only (the
# sandbox has no R and CRAN is blocked), the Ch 8 DuckDB / Ch 9 fd
# / Ch 11 ShellCheck pattern. ALL commands are ordinary
# non-interactive Rscript/uv invocations (no interactive shell is
# driven, so nothing hangs on a real terminal):
#   1. ch13-renv-mac.txt - renv::status() and a no-op
...
#   2. ch13-uv-mac.txt   - uv --version on the Mac, so the
#      chapter's sandbox uv version stamp can be reconciled
#      against the Mac install.
#
# Run on the Mac, from the terminal repo root
# (source-private/terminal), the way ch08/ch09/ch10/ch11 were run:
#   bash transcripts/capture-ch13-mac.sh
# then review the two ch13-*-mac.txt files it writes and paste
...
# transcripts/README.md; same helper as capture-ch11-mac.sh incl.
...
rout="$here/ch13-renv-mac.txt"
uout="$here/ch13-uv-mac.txt"
