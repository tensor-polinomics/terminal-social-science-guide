#!/usr/bin/env bash
# Capture the Ch 4 (Navigation & file operations) macOS/BSD
# facts on the user's Mac.
#
# Ch 4's portable/GNU output is captured in the book's Linux
# sandbox (ch04-*.txt). This script captures only the BSD
# divergences a real Mac shows, which the sandbox cannot:
#   1. ch04-home-mac.txt    - the ~ / $HOME shorthand resolving
#      to /Users/<you> (shown from the Mac, as in Ch 2, because
#      the sandbox home is a /sessions quirk).
#   2. ch04-ls-mac.txt      - BSD `ls -l` (the `total` line is
...
#   3. ch04-stat-mac.txt    - BSD `stat` default output and the
#      BSD format flag `-f` (GNU uses `-c`, a different syntax).
#   4. ch04-symlink-mac.txt - BSD `ln -s`, `ls -l` arrow line,
...
#   5. ch04-case-mac.txt    - the default macOS filesystem is
#      case-INSENSITIVE: `Foo.txt` and `foo.txt` are the SAME
#      file (on Linux they are two files, ch04-case.txt).
#
# Run on the Mac, from the terminal repo root
# (source-private/terminal), the way ch02/ch03 were run:
#   bash transcripts/capture-ch04-mac.sh
# then review the five ch04-*-mac.txt files it writes and paste
...
# transcripts/README.md). Ch 4 is the first chapter to show
...
hout="$here/ch04-home-mac.txt"
lout="$here/ch04-ls-mac.txt"
sout="$here/ch04-stat-mac.txt"
yout="$here/ch04-symlink-mac.txt"
cout="$here/ch04-case-mac.txt"
...
scratch="$(mktemp -d "${TMPDIR:-/tmp}/ch04mac.XXXXXX")"
...
# --- 4. BSD symlink: ln -s, readlink, readlink -f / realpath --
{
  hdr "BSD ln/readlink/realpath (macOS base)" \
    ...
    "machine identifier); the ch04mac.XXXXXX suffix is a" \
    "random mktemp tag, not identifying."
  cd "$scratch" || exit 1
...
# --- 5. case-insensitivity on the default macOS filesystem ----
{
  hdr "APFS default (case-insensitive)" \
    ...
    "entry. On Linux (ch04-case.txt) they are two files. If" \
