#!/usr/bin/env bash
# Ch 18 remote-shell startup seam: "works in terminal, fails over
# SSH". Captured from the Mac against the user's real Linux server
# (the same class of box as Ch 14). A non-interactive
...
#   bash transcripts/capture-ch18-ssh-mac.sh <user@host>
# e.g.  bash transcripts/capture-ch18-ssh-mac.sh lab-server
#
...
today="$(date +%F)"
out="$here/ch18-ssh-seam-mac.txt"
