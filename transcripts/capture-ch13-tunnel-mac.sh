#!/usr/bin/env bash
# Re-capture ONLY the Ch 13 ssh -L port-forward demo.
#
# The main capture-ch13-mac.sh runs the tunnel inline as
...
#   bash transcripts/capture-ch13-tunnel-mac.sh <ssh-alias>
#
# It writes transcripts/ch13-tunnel-mac.txt (overwriting the empty
# one). Server identifiers are derived live and masked; the only
# content is a demo directory listing. It creates and deletes a
# throwaway ~/ch13-tunnel-demo on the server and kills both the
# server process and the local tunnel at the end.

set -uo pipefail

SERVER="${1:?usage: bash transcripts/capture-ch13-tunnel-mac.sh <ssh-alias>}"
...
ssh "${SSHO[@]}" "$SERVER" "rm -rf ~/${demo} && mkdir -p ~/${demo} \
  ...
  && (nohup timeout 30 python3 -m http.server ${RP} \
       >/tmp/ch13hs.log 2>&1 &) ; sleep 1" >/dev/null 2>&1
...
echo "captured -> $here/ch13-tunnel-mac.txt"
echo "----------"
cat "$here/ch13-tunnel-mac.txt"
