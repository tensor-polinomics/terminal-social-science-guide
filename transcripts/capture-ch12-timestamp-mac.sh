#!/usr/bin/env bash
# Deterministic timestamp demo for Ch 12 ("When timestamps
...
# real macOS race (ch12-make-mac.txt) is the evidence for
# 3.81's one-second-truncation hazard.
#
# Run on the Mac, from anywhere:
#   bash source-private/terminal/transcripts/capture-ch12-timestamp-mac.sh
# then review/commit transcripts/ch12-timestamp-mac.txt.
#
...
out="$here/ch12-timestamp-mac.txt"
