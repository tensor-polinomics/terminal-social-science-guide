...
- **Wrapped commands: hand-wrap with `\`, no `>` (match the book).** When a
  ...
  raw capture and may retain the real `>` (e.g. `ch05-stat-mac.txt`) or not
  (e.g. `ch16-bat-mac.txt`). Output must match byte-for-byte; the `\`-wrap
...
- `05-navigation.qmd`: `echo $HOME`/`echo ~` (`ch05-home-mac`).
- `12-make-pipeline.qmd`: 3 blocks (`touch`/`make -n`; two
  `touch`/`stat`/`make`) (`ch12-make-mac`, `ch12-timestamp-mac`).
...
- `16-modern-cli-tuis.qmd`: `bat` block (3 line-number lines regained a
  ...
  space) (`ch16-bat-mac`, `ch16-diff-mac`).
...
- `12-make-pipeline.qmd`: the `sed`/`make check` failure block; the `sed`
  fit on one line (matching `ch12-make-mac`), and the two dropped blanks
...
`transcripts/ch11-primer.txt` was updated to match; output is unchanged
...
- `05-navigation.qmd` `stat -f '%N %z ...'` block: the chapter hand-wraps
  the command with `\` and no `>`; `ch05-stat-mac.txt` retains the real
...
- **`12-make-pipeline.qmd` `stat -f '%Fm  %N' output/tables/ff5_alpha.tex
  ...
  `transcripts/capture-ch12-mtime-mac.sh` (forced `make -B report.html`,
  then `stat`), producing `transcripts/ch12-mtime-mac.txt` (macOS 26.5.2,
...
- **`$ make` full-build block.** Restored the dropped `months=179
  ...
  contiguous slice of `ch12-make-mac.txt`.
...
- **`make output/figures/cumret.png` Python-subgraph block.** Its single
  ...
  genuinely-elided output runs. Honest slice of `ch12-python-subgraph.txt`.
- **`touch scripts/04_regression.R; make` block -- SPLICE, RE-CAPTURED.**
  ...
  the report. The original `ch12-make-mac.txt` capture shows `touch; make`
  ...
  `transcripts/capture-ch12-touch-regression-mac.sh` does a `make` to
  ...
  `transcripts/ch12-touch-regression-mac.txt` (macOS 26.5.2). The chapter
