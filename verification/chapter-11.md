...
- **`verify.sh` anchor** (`ch10-verify.txt`): the file-level
  ...
  (Chapter 12), and the chapter says so rather than conflating
  them.
...
- **Ch 8** `LC_ALL` pin is delivered as part of the standard
  header; the chapter references, not re-derives, Ch 8's
  locale/sort demo.
- **Ch 12 `make check`** is cross-referenced as the content-hash
  sibling of `verify.sh`'s file-level check; Make mechanics are
  not taught (the script is the node, Make is the graph).
- **Ch 10** `chmod +x`, `>&2`, and the `SIGTERM` cleanup handler
  are referenced (permissions, stderr, trap-as-cleanup), not
  re-taught. Locale/`TZ` that Ch 10 forward-referenced to Ch 11
  is delivered.
...
- **Bash Reference Manual, The Set Builtin (`-e`, `-u`,
  ...
  pages, also seen at Ch 10); the `set`/`trap` behavior the chapter
...
- **util-linux `script`** records a full session into a
  ...
  the way Ch 10 handled interactive-only features. `tee` is the
