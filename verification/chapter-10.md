# Source-verification log: Chapter 10 (Processes, permissions, secrets)

Per PLAN.md Section 10. Chapter 10 is executable: the process,
...
- **Ch 5's deferral is honored.** Ch 5 said reading the `rwx`
  string is its job and "Changing permissions with `chmod`, the
  octal shorthand like `644` and `755`, and who owns a file are
  Chapter 10's subject." Ch 10's permissions and ownership sections
  deliver exactly that, applying (not re-teaching) Ch 5's reading
  of the triples. Present tense (Ch 5 committed `3d5a62b`).
...
- **Ch 7's danger has a sibling, not a re-teach.** The secret-leak
  DANGER is framed as the "reveal it" sibling of Ch 7's "destroy
  it"; `rm -rf`/`>`/`curl | sh` and quoting/null-safety are
  referenced, not repeated. The `chmod 777` DANGER points to Ch 7
  the way that chapter treats `curl | sh`. Present tense (Ch 7
  committed `22eec6c`).
- **Ch 6's `.gitignore` rule is the anchor.** The secrets stanza
  read from the project's own `.gitignore` (`.env`, `*.pem`,
  `secrets.*`) is the one Ch 6's layout set. Present tense (Ch 6
  committed `6d46e79`).
- **Git book owns "never commit a secret"; Ch 10 owns the rest.**
  Cross-referenced (never committed; purge from history in the
  RECOVERY), not re-argued (PLAN Section 6).
- **Ch 17 (AI prompts) cross-ref, present tense (committed
  ...
  printed secret lands. Ch 17 already cross-refs Ch 10 for exactly
  this, so the two are consistent.
- **Ch 11 (tee/script logs, `set -euo pipefail`, locale),
  Ch 14 (SSH/remote long jobs, `~/.ssh` keys), Ch 15 (tmux
  persistent reattachable sessions), Ch 18 (startup files)** are
  referenced with forward "will" language appropriate to reading
  order; Ch 11, 14, and 15 are now drafted and committed (Ch 18
  remains a stub), so the "will" is a forward pointer for a
  reader at Ch 10, not a claim the target is unwritten. The
  `nohup` section explicitly draws the line to tmux (Ch 15) and
  remote work (Ch 14) as the fuller answers, both now delivered.
...
more naturally Ch 14's). Shown `$` blocks: 14 sandbox blocks
...
passed on the Mac (153-page PDF, Ch 10 pp. 105-117, Ch 11 starts
