# Source-verification log: Chapter 14 (SSH and remote compute)

Per PLAN.md Section 10. Chapter 14 is unusual for this book:
...
- **`renv::restore()` rebuilds the R library on the server**
  ...
  Chapter 8's committed value. `lattice`/`nlme` show
...
- **Ch 10** taught `chmod` and `nohup`; Section 2 APPLIES `600`/
  `700` to key files (not re-teaching bit-reading), and Section
  6 cross-refs `nohup` as the disconnect stopgap. The env-var
  secrets and `ps`/history leak vectors stay Ch 10's; SSH-agent
  handling is this chapter's own.
- **Ch 11** built a `sha256sum` manifest; Section 5 applies the
...
- **Ch 13** ended on the promise that `uv sync`/`renv::restore()`
...
- **Ch 16** owns the modern TUI kit; `ncdu` is named and routed
  there rather than shown (it is an interactive TUI, not a
  one-shot command).
- **Ch 18** is forward: the non-interactive-`ssh` `PATH` note
  (`~/.local/bin/uv` named in full) points at Ch 18's
...
- **renv R-version behavior** (per the renv docs pinned in
  Chapter 13's verification log: renv TRACKS but does not MANAGE
