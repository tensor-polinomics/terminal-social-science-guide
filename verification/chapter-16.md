# Source-verification log: Chapter 16 (AI in the Terminal)
...
- chapter/section: Ch 16, "Permission models"; DANGER
...
- chapter/section: Ch 16, "The destructive-command boundary";
...
- verifies: writes to protected paths (`.git`, `.claude`, shell
  ...
  Ch 6, plus tool guardrails that are real but not a substitute
  for review.
...
- chapter/section: Ch 16, "Permission models"; "File and scope
...
- chapter/section: Ch 16, "Network access and the supply chain";
  DANGER
- source: "Agent approvals & security" - Codex, OpenAI Developers
  (https://developers.openai.com/codex/agent-approvals-security)
- accessed: 2026-07-01
- verifies: "By default, the agent runs with network access turned
  ...
  agent with network access can run exactly the Ch 6 `curl | sh`.
- version note: current as of 2026-07-01
- confirmable: yes

### Gemini CLI: approval modes (default / auto_edit / yolo / plan) and sandbox
- chapter/section: Ch 16, "Permission models"; "What an agent
...
- chapter/section: Ch 16, "Secrets in prompts"; PITFALL
...
- chapter/section: Ch 16, "Review discipline"; "Reproducibility
...
- source: this book, Ch 6 (`curl | sh`, `rm -rf`, PATH-hijack)
  and Ch 9 (secrets, shell history, `HISTFILE`); companion Git
  book (review discipline, diffs, replication package)
- accessed: n/a
- verifies: the chapter cross-references, and does not re-teach,
  the Ch 6 danger material and the Ch 9 secrets material; an agent
...
- **Version anchor: DONE (2026-07-01).** `capture-ch16-mac.sh`
  ...
  in `--help`. The chapter states versions in prose (Ch 11 style);
...
- **Doc fetches: DONE (2026-07-01).** All three official docs
  ...
  dead-host / Internet-Archive fallback was needed for Ch 16.
