# Source-verification log: Chapter 4 (Navigation & file operations)

Per PLAN.md Section 10. Chapter 4 is executable: nearly all of
...
Forward references: Ch 2, 3, 6, 11 are drafted and cited in
present tense / scope form; Ch 5, 9, 10, 17 and Appendix A are
stubs and every reference to them is a scope statement ("the
subject of Chapter 9", "Appendix A collects...") or
forward-contract language ("Chapter 5 will lean on it",
"Chapter 10 will return to this", "Chapter 17"), never a claim
they already teach or verify anything. (Appendix A is cited in
the present-tense "collects" form to match the committed Ch 6,
...
- source: live sandbox captures `ch04-ls-long.txt`
  ...
  octal 644/755, `chown`) stays Ch 9's job (stub, scope form).
...
- verifies: exactly what is shown; `mkdir -p` idempotence noted
  and routed forward to Ch 5 scaffolding (stub, "will"); the
  contrast with `rm -r` routed to Ch 6 (drafted)
...
- source: live sandbox capture `ch04-cp-mv.txt`: `cp`, `cp -r`,
  ...
  the transcript note and the chapter). DANGER points to Ch 6
  (owns `>`/`rm -rf`, the `${VAR:?}` guard) and Appendix A;
  RECOVERY cross-refs `git restore` (Git book), `make`
  regeneration (Ch 11), and the no-recovery case.
- accessed: 2026-07-02 (capture date)
- verifies: cp/cp -r/mv behavior, the silent overwrite, the
  `-i` guard; no runnable `rm` demo here (Ch 6 owns the
...
- source: Linux side live `ch04-case.txt` (`Foo.txt` and
  ...
  Ch 10 (locale/line-endings, stub, "will").
- accessed: 2026-07-02 (sandbox + Mac reconcile)
...
- chapter/section: Ch 4 throughout
- source: this book, Ch 2 (working directory, absolute-vs-
  relative, `cd` is a builtin, the BSD/GNU split, `$HOME` shown
  from the Mac), Ch 3 (a working shell on your platform), Ch 6
  (owns `rm -rf` / `>` clobber / `${VAR:?}`; Appendix A), Ch 11
  (`make` mtime comparison; regeneration); forward to Ch 5
  (scaffolding), Ch 9 (permissions), Ch 10 (locale/case/line
  endings), Ch 17 (aliases in startup files); Appendix A (panic
...
- verifies: the chapter delivers Ch 1/2's stated Ch-4
  ...
  on Ch 2's model) and sets up rather than steals Ch 5/6's jobs
- confirmable: yes (internal)

## Gate confirmations

- **Validator: 0 errors / 0 warnings** in the sandbox (whole
  book and on `book/chapters/04-navigation.qmd` alone, via a
...
- **Counts:** 7 numbered content sections (5 beginner, 2
  ...
  DANGER (short, points to Ch 6 + App A), 1 PITFALL
