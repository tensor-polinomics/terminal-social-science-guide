# Source-verification log: Chapter 12 (Environments from the shell)

Per PLAN.md Section 10. Chapter 12 is largely executable on the
...
Ch 7 DuckDB / Ch 8 `fd` / Ch 10 ShellCheck pattern; two
...
- **Mac-backed, RECONCILED byte-for-byte 2026-07-04 (2 blocks,
  ...
  Ch 7/8/10 lesson): the R 4.5.3 release date is **2026-03-11**
  ...
  check, the class of the Ch 5 box-glyph lesson, since the
  ...
  `ch12-lockread.txt`), consistent with Chapter 7's committed
...
- **Authored, non-capture:** the `.venv/` annotated layout tree
  ...
  Ch 5 house device, ASCII connectors), paired with the real
...
- **The manifest declares ranges, the lock records exact
  ...
  (36 packages, agreeing with Chapter 7's committed jq reads).
...
- **Ch 2 / Ch 9 shell environment vs language environment:**
  section 1 explicitly disambiguates the two meanings of
  "environment"; `PATH`/`export` (Ch 2) and env-var secrets
  (Ch 9) are referenced, not re-taught.
- **Ch 5** named `pyproject.toml`/`uv.lock`/`renv.lock` in the
  layout and deferred operational use to Ch 12; delivered here.
  The commit-the-lock rule applies Ch 5's tracked-vs-regenerated
  line, not a new rule.
- **Ch 7** used `renv.lock` as `jq` material and deferred its
  meaning to Ch 12; delivered (same file, same tool, new
  question). The jq reads here (R 4.5.3, 36 packages' stack,
  fixest 0.14.2) are consistent with Ch 7's committed values.
- **Ch 10**: the run-log habit (`tee`, dated filenames) is
  extended to environment receipts; the REPRODUCIBILITY callout
  assigns each Part-III layer its own pin (Ch 10 shell env,
  Ch 12 language env, Ch 11 orchestration + `make check`)
  without crediting any layer for another's work.
- **Ch 11**: `PY := uv run python` and the `Rscript` node are
  ...
  the alpha-locked-at-4dp caveat is attributed to Ch 11's
  record.
- **Ch 4** `readlink` is applied (the `.venv/bin/python`
  symlink), not re-taught.
- **Ch 13 (stub)** stays forward: "which Chapter 13 will lean
...
- **Callouts: 1 PITFALL, 1 REPRODUCIBILITY, 1 DIVERGENCE.**
  ...
  Ch 5), no RECOVERY (the recovery IS the restore verbs, taught
...
- Mechanical: `grep -E '^## ' | grep -v 'Try it yourself' | wc
  ...
  re-review (Codex, 2026-07-04: 177-page PDF, Ch 12
  pp. 145-156, Ch 13 starts p. 157, no clipping, the §5 curly
