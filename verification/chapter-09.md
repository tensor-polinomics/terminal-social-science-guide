# Source-verification log: Chapter 9 (Finding things)

Per PLAN.md Section 10. Chapter 9 is executable: nearly all of
...
- **Real, user's Mac (macOS/BSD `find` divergences + `fd`),
  ...
  (the Ch 8 DuckDB version lesson; fixed in the chapter). The
  ...
  3. `fd --version` - CHANGED on reconcile to `fd 10.4.2`
     ...
     real capture, the Ch 8 DuckDB v1.4.1 -> v1.5.4 lesson).
...
- **Ch 8's forward promise is delivered.** Ch 8 said "Chapter 9
  ...
  Present tense (Ch 8 is committed, `10cc8b0`).
- **Ch 7 null-safety is REINFORCED, not re-taught.** The
  `find -print0 | xargs -0` block points back to Ch 7's rule
  (present tense; Ch 7 committed `22eec6c`); pipes/quoting
  themselves are not re-explained.
- **Ch 6 raw-data immutability** is cross-referenced in the
  DANGER callout (present tense; Ch 6 committed `6d46e79`): the
  `.gitignore` that hides `data/` is the one Ch 6 set, and the
  look-before-you-leap habit ties to Ch 6's immutability rule
  and Ch 7's clobber hazard.
- **Ch 5** `ls -l`/`stat` metadata is referenced (present
  tense; committed `3d5a62b`) as the properties `find -size`
  and `-mtime` select on.
- **Ch 14 (rsync, servers)** referenced with forward "will"
